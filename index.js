...
app.get("/", async (req, res) => {
  ...
  if (query) {
    let cityRes = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${query}&appid=${process.env.OPENWEATHER_API_KEY}&units=metric`);

    let city = await cityRes.json();
